...
begin
  ...
  --
  -- ON CONFLICT DO NOTHING tolerates a pre-existing row from before
  -- this migration (rare orphan from the legacy non-atomic flow).
  -- Idempotent retries on the same task_id are caught earlier by the
  -- tasks unique violation, so this branch is effectively only the
  ...
  on conflict on constraint queue_task_deadlines_pkey do nothing;
end
```
